...
{
    public class Security_UpdateObjectPermissionQuerySetter : QueryInitializerBase
    {
        public override string CollectionName { get { return "Security_UpdateObjectPermission"; } }
        protected override void Do()
        {
            Register<GetTypeQueryBase>(c => typeof(UpdateObjectPermission));

            Register<GetIdentifierQueryBase>(inp => (inp.Object as Role).Id.ToString());

            Register<LookupListQueryBase>((inp =>
            {
                var allEntitiesWithGetListQuery = QueryResolver.GetAllKeys().Where(key => key.ObjectType == typeof(UpdateObject)).Select(k => k.CollectionName);
                
                var query = allEntitiesWithGetListQuery.Select(ent => new UpdateObjectPermission
                    {
                        Collection = ent
                    });
            ...
            Register<ToManyGetQueryBase>((inp =>
            {
                ...
                inp.Output = 
                    IoC.SecurityProvider.Model.Roles.First(p => p.Id == roleId).Permissions
                       .Where(p => p.GetType() == typeof(UpdateObjectPermission))
            ...
            Register<ToManySave>((inp =>
            {
                ...
                role.Permissions.RemoveAll(p => p.GetType() == typeof(UpdateObjectPermission));

                foreach (var entity in inp.Identifiers)
                {
                    role.Permissions.Add(new UpdateObjectPermission
                        {
                            Collection = entity,
                            Grant = true
                        });
            ...
            Register<GetListCountQueryBase>((inp => QueryResolver.GetAllKeys().Count(key => key.ObjectType == typeof(UpdateObject))));
        }
    }
}