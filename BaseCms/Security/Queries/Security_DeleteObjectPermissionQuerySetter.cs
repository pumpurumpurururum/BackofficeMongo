...
{
    public class Security_DeleteObjectPermissionQuerySetter : QueryInitializerBase
    {
        public override string CollectionName { get { return "Security_DeleteObjectPermission"; } }
        protected override void Do()
        {
            Register<GetTypeQueryBase>(c => typeof(DeleteObjectPermission));

            Register<GetIdentifierQueryBase>(inp => (inp.Object as Role).Id.ToString());

            Register<LookupListQueryBase>((inp =>
            {
                var allEntitiesWithGetListQuery = QueryResolver.GetAllKeys().Where(key => key.ObjectType == typeof(DeleteObject)).Select(k => k.CollectionName);
                
                var query = allEntitiesWithGetListQuery.Select(ent => new DeleteObjectPermission
                    {
                        Collection = ent
                    });
            ...
            Register<ToManyGetQueryBase>((inp =>
            {
                ...
                inp.Output = 
                    IoC.SecurityProvider.Model.Roles.First(p => p.Id == roleId).Permissions
                       .Where(p => p.GetType() == typeof(DeleteObjectPermission))
            ...
            Register<ToManySave>((inp =>
            {
                ...
                role.Permissions.RemoveAll(p => p.GetType() == typeof(DeleteObjectPermission));

                foreach (var entity in inp.Identifiers)
                {
                    role.Permissions.Add(new DeleteObjectPermission
                        {
                            Collection = entity,
                            Grant = true
                        });
            ...
            Register<GetListCountQueryBase>((inp => QueryResolver.GetAllKeys().Count(key => key.ObjectType == typeof(DeleteObject))));
