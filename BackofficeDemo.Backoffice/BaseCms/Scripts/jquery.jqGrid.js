...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var eg = $("<div class='ui-jqgrid dataTables_wrapper'></div>");
		...
		addMulti = function(rowid,pos,irow,checked){
		    var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/><span class=\"lbl\"></span>",
		...
		setPager = function (pgid, tp){
			...
			if(dir=="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
			if (ts.p.viewrecords === true) { $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append("<div dir='" + dir + "' style='padding-right: 10px; text-align:" + ts.p.recordpos + "' class='dataTables_info ui-paging-info'></div>"); }
		...
		if(this.p.multiselect) {
		    this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/><span class=\"lbl\"></span>");
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$(this).addClass('ui-jqgrid-btable table table-striped table-bordered table-hover dataTable').append(firstr);
		firstr = null;
	    var hTable = $("<table class='ui-jqgrid-htable table table-striped table-bordered table-hover dataTable' style='width:" + ts.p.tblwidth + "px' role='grid' aria-labelledby='gbox_" + this.id + "' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({ width: grid.width + "px" }).addClass('row').appendTo(eg);
