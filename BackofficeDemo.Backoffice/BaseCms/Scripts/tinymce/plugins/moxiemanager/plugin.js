		...
		editorSettings.file_browser_callback = function(id, value, type, win) {
			...
			Loader.browse(tinymce.extend({
				...
				oninsert: function(args) {
					...
					//var path = args.focusedFile.path;
					//var firstSlash = path.indexOf('/');
					//var secondSlash = path.indexOf('/', firstSlash + 1);

					//var pas = '/BaseCms/Scripts/tinymce/plugins/moxiemanager/data/';
					//var convertUrl = pas + path;//editor.settings.imagesUrl;
					//if (convertUrl[convertUrl.length - 1] != '/') convertUrl = convertUrl.trim('/');

				    //fieldElm.value = convertUrl;// + path.substring(secondSlash, path.length);
				    ...
				    var convertUrl = editor.settings.imagesUrl;
				    if (convertUrl[convertUrl.length - 1] != '/') convertUrl = convertUrl.trim('/');
