...
(function($ , undefined) {//ACE
	...
	var Tree = function (element, options) {
	   ...
	   // this.$element.on('click', '.ace-icon.fa.fa-times', $.proxy( function(ev) { this.selectItem(ev.currentTarget); } ,this));
		
		//this.$element.on('click', '.tree-folder .tree-folder-header .ace-icon', $.proxy(function (ev) {
		//    this.selectFolder(ev.currentTarget);
		//    ev.stopPropagation();
		//}, this));

		this.$element.on('click', '.ace-icon.fa.fa-times', $.proxy(function (ev) {
		    this.selectItem(ev.currentTarget);
		    ev.stopPropagation();
		}, this));
		this.$element.on('click', '.ace-icon.fa.fa-check', $.proxy(function (ev) {
		    this.selectItem(ev.currentTarget);
		    ev.stopPropagation();
		}, this));
		...
		this.$element.on('click', ' .tree-folder-header', $.proxy(function (ev) {
		   // this.selectFolder(ev.currentTarget);
		    this.itemClick(ev.currentTarget);
		    ev.stopPropagation();
		}, this));
		...
		this.$element.on('click', '.tree-folder .tree-folder-header > i', $.proxy(function (ev) {
		    this.selectFolder(ev.currentTarget);
		    ev.stopPropagation();
		}, this));
	...
	Tree.prototype = {
			...
			this.options.dataSource.data($el.data(), function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if ($el.hasClass('tree-folder-header')) {
					    ...
					    //if ($(".ace-icon.tree-plus").length > 0)
					    //    $(".ace-icon.tree-plus")[0].click();
					    
					    //this.on('click', ' .tree-folder-header', $.proxy(function (ev) {
					    //    // this.selectFolder(ev.currentTarget);
					    //    this.itemClick(ev.currentTarget);
					    //    ev.stopPropagation();
					    //}, this));
		...
		//		$el.find('ace-icon.fa.fa-times').removeClass('ace-icon.fa.fa-times').addClass('ace-icon.fa.fa-check');
		//	} else {
		//		$el.addClass ('tree-selected');
		//		$el.find('ace-icon.fa.fa-check').removeClass('ace-icon.fa.fa-check').addClass('ace-icon.fa.fa-times');
		//		if (this.options.multiSelect) {
		//			data.push( $el.data() );
		//		}
		//	}
		...
		selectItem: function (el) {
		    ...
		    if (this.options.multiSelect) {
		        $.each($all, function (index, value) {
		            var $val = $(value);
		            if ($val[0] !== $el[0]) {
		                data.push($(value).parent().data());
		            }
		        });
		    } else if ($all[0] !== $el[0]) {
		        $all.removeClass('tree-selected')
	                .find('i').removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);
		        data.push($el.parent().data());
		    }

		    if ($el.hasClass('tree-selected')) {
		        $el.removeClass('tree-selected');
		        //$el.find('i').removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);
		        $el.removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);
		        $el.attr("style", "");
		    } else {
		        $el.addClass('tree-selected');
		        $el.attr("style", "color: #F3F4F8; background-color: #F1A80C;");
		        $el.removeClass(this.options['unselected-icon']).addClass(this.options['selected-icon']);
		    ...
		    if (data.length) {
		        ...
		        this.$element.trigger('selected', { info: 0 });
		    }
		...
		itemClick: function (el) {
		    console.log(el);
		    this.$element.trigger('click', $(el).data());
		},
